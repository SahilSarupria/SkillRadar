...
from .models import (
    ...
    UserRecommendation, LearningPath, UserProgress, AffiliateClick
)

@admin.register(LearningPlatform)
class LearningPlatformAdmin(admin.ModelAdmin):
    list_display = ['name', 'is_active', 'total_courses', 'last_sync_at', 'created_at']
...
class PlatformIntegrationAdmin(admin.ModelAdmin):
    list_display = ['platform', 'is_configured', 'last_sync_at']
...
class UserRecommendationAdmin(admin.ModelAdmin):
    list_display = ['user', 'course_title', 'target_skill', 'recommendation_type', 'relevance_score', 'priority', 'is_bookmarked', 'is_completed']
    list_filter = ['recommendation_type', 'priority', 'is_bookmarked', 'is_completed', 'created_at']
...
class LearningPathAdmin(admin.ModelAdmin):
    list_display = ['title', 'user', 'difficulty_level', 'estimated_weeks', 'estimated_cost', 'relevance_score']
    list_filter = ['difficulty_level', 'created_at']
    search_fields = ['title', 'user__email', 'description']
    readonly_fields = ['created_at', 'updated_at']
    filter_horizontal = ['skills', 'recommended_courses']
    
    def get_queryset(self, request):
        return super().get_queryset(request).select_related('user').prefetch_related('skills', 'recommended_courses')

@admin.register(UserProgress)
class UserProgressAdmin(admin.ModelAdmin):
    list_display = ['user', 'learning_path_title', 'progress_percentage', 'is_completed', 'started_at']
