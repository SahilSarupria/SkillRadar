...
from .models import LearningPlatform, Course, Skill, CourseRecommendation
import logging
...
def generate_user_recommendations_task(self, user_id, skill_gaps=None, learning_goals=None):
    ...
    try:
        ...
        for rec in recommendations:
            user_rec, created = CourseRecommendation.objects.update_or_create(
                user=user,
                course=rec['course'],
                defaults={
...
def update_course_popularity_scores_task():
    ...
    try:
        ...
        for course in courses:
            ...
            recommendation_count = CourseRecommendation.objects.filter(course=course).count()
            bookmark_count = CourseRecommendation.objects.filter(course=course, is_bookmarked=True).count()
            completion_count = CourseRecommendation.objects.filter(course=course, is_completed=True).count()
...
def cleanup_old_recommendations_task():
    ...
    try:
        ...
        old_recommendations = CourseRecommendation.objects.filter(
...
def send_recommendation_notifications_task():
    ...
    try:
        ...
        recent_recommendations = CourseRecommendation.objects.filter(
